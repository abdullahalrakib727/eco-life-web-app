...
const OurBlogs: FC = (): JSX.Element => {
  return (
    <section className="bg-[#D6F7E7] relative h-screen">
      ...
      <div>
        <div className="mt-12 px-10 grid grid-cols-1 md:grid-cols-3 gap-5">
          {Blogs.map((blog) => (
            <div className="bg-white rounded-2xl border-2 shadow-md border-primary-color flex flex-col items-center" key={blog.id}>
              <img
                src={blog.image}
                alt=""
                className="max-w-[334px] max-h-[184px] mt-14"
              />
              <div className="mb-16">
              <h1 className="mt-5 text-center text-primary-color font-semibold text-2xl px-5">{blog.title}</h1>
              <p className="px-2 mt-14 text-secondary-color text-center">{blog.description}</p>
              </div>
